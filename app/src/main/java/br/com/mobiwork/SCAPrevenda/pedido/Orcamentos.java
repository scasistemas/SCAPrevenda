 ...
 */
public class Orcamentos  extends Activity  implements AsyncResponse {
    ...
    public void selecionarVisualizarPedido(){
        if (!isEditPedido) {
            Intent i = new Intent(Orcamentos.this,pedidovendateste.class);
            i.putExtra("PEDIDO_ID",cursor.getString(cursor.getColumnIndex("numero")));
            i.putExtra("tipopre","orcamento");
            Orcamentos.this.startActivityForResult(i,100);
            isEditPedido = true;
        }
    }
    ...
    public void enviar(){
        if (!isEditPedido) {
            sincup = new SincUp(Orcamentos.this,pv,"",config);
            sincup.delegate = Orcamentos.this;
    ...
    public void popularLista(boolean paramvend) {
        if(config.getEmp()!=null){
            ...
            if(config.getEmp().equalsIgnoreCase("jrc")&&paramvend==false){
                ...
                cursor =dpv.orcenvi(db,param,va);
            }else{
                cursor =dpv.orcenvi(db,param);
            }
